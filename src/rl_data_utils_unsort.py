...
class RLDataUtilUnsort(object):
  def __init__(self, hparams, shuffle=True):
    ...
    for i in range(len(self.lan_i2w)):
      self.lan_dist_vec.append(self.query_lan_dist(self.hparams.base_lan_id, i) / 100)
    self.lan_dist_vec = np.array(self.lan_dist_vec)

  def load_nmt_train_prob_lr(self, src, trg):
    ...
    for trg_idx, trg_line in enumerate(trg_file):
      ...
      while True:
        ...
        if src_line == "EOF":
          break
        if src_line and prob[i] >= self.hparams.min_lr_prob:
          line_count += 1
          ...
          self.prob_train.append(prob[i])
        i += 1
    (self.x_train, self.y_train, self.prob_train), _ = self.sort_by_xlen([self.x_train, self.y_train, self.prob_train])
  ...
  def load_nmt_train(self, src, trg):
    ...
    (self.x_train, self.y_train, self.lan_id), _ = self.sort_by_xlen([self.x_train, self.y_train, self.lan_id])
    print("loading={}, total={}".format(line_count, total_count))

  def load_rl_train(self, src, trg, trg_lines):
    lan_src_counts = [0 for _ in range(self.hparams.lan_size)]
    sampled_indices = np.random.permutation(np.arange(len(trg_lines)))[:self.hparams.agent_subsample_line]
    self.data_raw_trg = []
    ...
    self.data_raw_src = [[[] for _ in range(self.hparams.lan_size)] for _ in range(len(sampled_indices))]
    self.data_src_len = [[0 for _ in range(self.hparams.lan_size)] for _ in range(len(sampled_indices))]
    cur_num, cur_lan = 0, 0
  ...
  def load_raw_data_list(self, src, trg, trg_lines):
    ...
    for trg_line in trg_lines:
      ...
      t_list.append(self.hparams.eos_id)
      self.data_raw_trg.append(t_list)
    self.data_raw_src = [[[] for _ in range(self.hparams.lan_size)] for _ in range(len(self.data_raw_trg))]
    ...
    while True:
      ...
      for data_idx in idxes:
        ...
        while end_index < len(x_dev_len):
          ...
          x_rank = []
          if self.shuffle:
            (x, y, x_char, dev_file_index, x_rank), _ = self.sort_by_xlen([x, y, x_char, dev_file_index, x_rank])
          # pad
    ...
    while True:
      for data_idx in range(len(self.hparams.test_src_file_list)):
        ...
        while end_index < len(x_test_len):
          ...
          if self.shuffle:
            (x, y, x_char, test_file_index), _ = self.sort_by_xlen([x, y, x_char, test_file_index])
  ...
  def _build_parallel(self, src_file_name, trg_file_name, data_idx, is_train=True, shuffle=True, outprint=False, sample=False, max_line=None, load_full=True):
    ...
    if is_train and shuffle:
      (src_data, trg_data, src_char_kv_data, src_word_rank), _ = self.sort_by_xlen([src_data, trg_data, src_char_kv_data, src_word_rank], descend=False)
    if outprint:
