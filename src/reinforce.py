...
class ReinforceTrainer():
    ...
    if self.hparams.load_model:
      ...
      if self.hparams.imitate_episode or self.hparams.epsilon_max:
        if self.hparams.imitate_type == "heuristic":
          ...
          print("actor not implemented")
          exit(0)
      elif self.hparams.not_train_score and not self.hparams.pretrained_actor:
        if self.hparams.imitate_type == "heuristic":
  ...
  def train_score(self):
    ...
    if self.hparams.train_on_loaded:
        ...
        for eps in range(self.hparams.train_score_episode):
          ...
          loss = (loss * grad_scale * self.hparams.reward_scale).masked_fill_(mask, 0.).sum().div_(self.hparams.train_score_every) 
  ...
  def train_nmt_full_bucketed(self):
    ...
    for (x_train, x_mask, x_count, x_len, x_pos_emb_idxs, y_train, y_mask, y_count, y_len, y_pos_emb_idxs, batch_size, x_raw, x_raw_len, y_raw, lan_selected_times, eop, bucket_instance_count) in self.data_loader.next_sample_nmt_train_bucketed(self.featurizer):
      ...
      cur_nmt_loss = torch.nn.functional.cross_entropy(logits, labels, ignore_index=self.hparams.pad_id, reduction="none").sum()
      total_loss += cur_nmt_loss.item()
