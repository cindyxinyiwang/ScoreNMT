...
class ReinforceTrainer():
  ...
  def train_score(self):
    ...
    # get gradient cosline similarity
  ...
  def imitate_heuristic(self):
    ...
    if self.hparams.imitate_type == "heuristic":
      heuristic_logits = Variable(torch.FloatTensor(self.data_loader.lan_dist_vec.tolist()) * self.hparams.hs_actor_temp)
    elif self.hparams.imitate_type == "init":
      heuristic_logits = [0 for _ in range(self.hparams.lan_size)]
      heuristic_logits[self.hparams.base_lan_id] = 100
      heuristic_logits = Variable(torch.FloatTensor(lan_vector))
    ...
    for eps in range(self.hparams.imitate_episode):
      for src, src_len, trg, iter_percent, eop in self.data_loader.next_raw_example_normal():
        ...
        a_target_logits = heuristic_logits 
        mask = 1 - s[1].byte()
        a_logits.masked_fill_(mask, -float("inf"))
