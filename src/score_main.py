...
parser.add_argument("--update_actor_every", type=int, default=100, help="frequency of updating actor")
...
def train():
  if args.load_model and (not args.reset_hparams):
    ...
    hparams = HParams(
      ...
      update_actor_every=args.update_actor_every,
    )
  # build or load model
  if hparams.decode:
    decode_prob_lr(hparams)
  else:
    train_nmt_prob_lr(hparams)

def decode_prob_lr(hparams):
  data_util = RLDataUtil(hparams)
  nmt_actor = torch.load(os.path.join(hparams.output_dir, "actor.pt"))
  
  if args.cuda:
    nmt_actor = nmt_actor.cuda()

  print("-" * 80)
  print("start decoding...")
  start_time = log_start_time = time.time()
  outfile = open(hparams.output_prob_file, 'w')
  step = 0
  for (x_train, x_mask, x_count, x_len, x_pos_emb_idxs, y_train, y_mask, y_count, y_len, y_pos_emb_idxs, batch_size, eop, index) in data_util.next_score_decode():
    src_empty = (np.array(x_len) == 2)
    scores = nmt_actor.forward(x_train, x_mask, x_len, x_pos_emb_idxs, y_train[:,:-1], y_mask[:,:-1], y_len, y_pos_emb_idxs).view(-1).data.cpu().numpy()
    scores[src_empty] = 0.
    scores = scores[index]
...
def train_nmt_prob_lr(hparams):
  ...
  nmt_actor = ActorNMT(hparams, model.encoder.word_emb, model.decoder.word_emb)
  ...
  for p in nmt_actor.parameters():
    p.data.uniform_(-args.init_range, args.init_range)
  trainable_params = [
    p for p in nmt_actor.parameters() if p.requires_grad]
  actor_optim = torch.optim.Adam(trainable_params, lr=hparams.lr, weight_decay=hparams.l2_reg)
  ...
  trainable_params = [
    p for p in nmt_actor.parameters() if p.requires_grad]
  num_params = count_params(trainable_params)
  print("Actor has {0} params".format(num_params))
  ...
  eval_actor(model, nmt_actor, actor_optim, data_util, crit, step, hparams, args, eval_bleu=False, valid_batch_size=args.valid_batch_size)	
  for (x_train, x_mask, x_count, x_len, x_pos_emb_idxs, y_train, y_mask, y_count, y_len, y_pos_emb_idxs, prob, batch_size, eop) in data_util.next_nmt_train_prob_lr():
    ...
    scores = nmt_actor.forward(x_train, x_mask, x_len, x_pos_emb_idxs, y_train[:,:-1], y_mask[:,:-1], y_len, y_pos_emb_idxs)
    ...
    if step % args.update_actor_every == 0:
      actor_grad_norm = torch.nn.utils.clip_grad_norm_(nmt_actor.parameters(), args.clip_grad)
      actor_optim.step()
    ...
    if eval_now:
      ...
      val_ppl, val_bleu, ppl_list, bleu_list = eval_actor(model, nmt_actor, actor_optim, data_util, crit, step, hparams, args, eval_bleu=based_on_bleu, valid_batch_size=args.valid_batch_size, tr_logits=logits)	
      for i in range(len(ppl_list)):
        ...
        if save or args.always_save:
          if len(ppl_list) > 1:
            nmt_save_checkpoint([step, best_val_ppl, best_val_bleu, cur_attempt, lr], model, optim, hparams, args.output_dir + "dev{}".format(i), nmt_actor)
          else:
            nmt_save_checkpoint([step, best_val_ppl, best_val_bleu, cur_attempt, lr], model, optim, hparams, args.output_dir, nmt_actor)
