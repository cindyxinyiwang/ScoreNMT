...
hparams_file_name = args.hparams_file
...
if not args.ppl_only:
  out_file_list = args.out_file_list.split(",")
...
hparams.test_ref_file_list = args.test_ref_file_list.split(",")
...
with torch.no_grad():
  ...
  for x, x_mask, x_count, x_len, x_pos_emb_idxs, y, y_mask, y_count, y_len, y_pos_emb_idxs, batch_size, x_char, y_char, eop, eof, test_file_idx, x_rank in data.next_test(test_batch_size=1):
    if hparams.model_type == 'seq2seq':
      hs = model.translate(
              x, x_mask, beam_size=args.beam_size, max_len=args.max_len, poly_norm_m=args.poly_norm_m, x_train_char=[], y_train_char=[], file_idx=test_file_idx)
  ...
  if args.trans_dev:
    valid_hyp_file_list = [os.path.join(hparams.output_dir, "{}_dev{}.trans_final".format(args.out_prefix, i)) for i in hparams.dev_file_idx_list]
    ...
    for x, x_mask, x_count, x_len, x_pos_emb_idxs, y, y_mask, y_count, y_len, y_pos_emb_idxs, batch_size, x_char, y_char, eop, eof, dev_file_index, x_rank in data.next_dev(dev_batch_size=1):
      hs = model.translate(
              x, x_mask, beam_size=args.beam_size, max_len=args.max_len, poly_norm_m=args.poly_norm_m, file_idx=[])
      for h in hs:
        h_best_words = map(lambda wi: data.trg_i2w[wi],
