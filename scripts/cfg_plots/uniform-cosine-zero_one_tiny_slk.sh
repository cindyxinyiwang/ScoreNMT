...
python src/rl_main.py \
  --clean_mem_every 5 \
  --reset_output_dir \
  --output_dir="outputs_plots/uniform-cosine-zero_one_tiny_slk/" \
  ...
  --dev_src_file_list  "$DDIR"data/slk_eng/ted-dev.mtok.spm8000.slk \
  --dev_trg_file_list  "$DDIR"data_moses/slk_eng/ted-dev.mtok.spm8000.eng \
  --dev_ref_file_list  "$DDIR"data_moses/slk_eng/ted-dev.mtok.eng \
  --dev_file_idx_list  "0" \
